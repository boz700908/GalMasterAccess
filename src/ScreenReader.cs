using System;
using System.Runtime.InteropServices;
using MelonLoader;
using UnityEngine;

namespace GalMasterAccess
{
    /// <summary>
    /// Wrapper for Tolk screenreader library.
    /// Provides simple methods to announce text via NVDA, JAWS, or other screenreaders.
    ///
    /// REQUIREMENTS:
    /// - Tolk.dll must be in the game folder (where the .exe is)
    /// - nvdaControllerClient64.dll (for 64-bit) or nvdaControllerClient32.dll (for 32-bit)
    ///   must also be in the game folder
    ///
    /// Download Tolk from: https://github.com/ndarilek/tolk/releases
    /// </summary>
    public static class ScreenReader
    {
        #region Native Imports

        [DllImport("Tolk.dll")]
        private static extern void Tolk_Load();

        [DllImport("Tolk.dll")]
        private static extern void Tolk_Unload();

        [DllImport("Tolk.dll")]
        private static extern bool Tolk_IsLoaded();

        [DllImport("Tolk.dll")]
        private static extern bool Tolk_HasSpeech();

        [DllImport("Tolk.dll", CharSet = CharSet.Unicode)]
        private static extern bool Tolk_Output(string text, bool interrupt);

        [DllImport("Tolk.dll")]
        private static extern bool Tolk_Silence();

        [DllImport("Tolk.dll", CharSet = CharSet.Unicode)]
        private static extern IntPtr Tolk_DetectScreenReader();

        #endregion

        #region Fields

        private static bool _available = false;
        private static bool _initialized = false;
        private static string _lastOutput;
        private static float _lastOutputTime = -1f;

        #endregion

        #region Public Methods

        /// <summary>
        /// Initializes Tolk. Call once at mod startup.
        /// </summary>
        public static void Initialize()
        {
            if (_initialized) return;

            try
            {
                Tolk_Load();
                _available = Tolk_IsLoaded() && Tolk_HasSpeech();

                if (_available)
                {
                    IntPtr srNamePtr = Tolk_DetectScreenReader();
                    string srName = srNamePtr != IntPtr.Zero
                        ? Marshal.PtrToStringUni(srNamePtr)
                        : "Unknown";
                    MelonLogger.Msg($"Screenreader detected: {srName}");
                }
                else
                {
                    MelonLogger.Warning("No screenreader detected or Tolk not available");
                }
            }
            catch (DllNotFoundException)
            {
                MelonLogger.Error("Tolk.dll not found! Place BOTH Tolk.dll AND nvdaControllerClient64.dll (or 32-bit version) in the game folder.");
                _available = false;
            }
            catch (Exception ex)
            {
                MelonLogger.Error($"Failed to initialize Tolk: {ex.Message}");
                _available = false;
            }

            _initialized = true;
        }

        /// <summary>
        /// Announces text via the screenreader.
        /// When Main.DebugMode is true, also logs all announcements via DebugLogger.
        /// </summary>
        /// <param name="text">Text to speak</param>
        /// <param name="interrupt">If true, stops current speech before speaking</param>
        public static void Say(string text, bool interrupt = true)
        {
            if (string.IsNullOrEmpty(text)) return;

            // A UI state can become visible and selected within the same update. Keep only
            // the first identical announcement from that transition.
            float now = Time.realtimeSinceStartup;
            if (text == _lastOutput && now - _lastOutputTime < 0.08f) return;
            _lastOutput = text;
            _lastOutputTime = now;

            // Log all announcements when debug mode is enabled
            DebugLogger.LogScreenReader(text);

            if (!_available) return;

            try
            {
                bool outputAccepted = Tolk_Output(text, interrupt);
                MelonLogger.Msg($"ScreenReader output {(outputAccepted ? "accepted" : "rejected")}: {text}");
            }
            catch (Exception ex)
            {
                MelonLogger.Warning($"ScreenReader.Say failed: {ex.Message}");
            }
        }

        /// <summary>
        /// Queued announcement - waits for current speech to finish before speaking.
        /// Use for additional info after a main announcement.
        /// Internally calls Tolk_Output(text, false).
        /// </summary>
        /// <param name="text">Text to speak after current speech finishes</param>
        public static void SayQueued(string text)
        {
            Say(text, false);
        }

        /// <summary>
        /// Stops current speech immediately.
        /// </summary>
        public static void Stop()
        {
            if (!_available) return;

            try
            {
                Tolk_Silence();
            }
            catch { }
        }

        /// <summary>
        /// Shuts down Tolk. Call when the game closes.
        /// </summary>
        public static void Shutdown()
        {
            if (!_initialized) return;

            try
            {
                Tolk_Unload();
            }
            catch { }

            _initialized = false;
            _available = false;
            _lastOutput = null;
            _lastOutputTime = -1f;
        }

        /// <summary>
        /// Returns true if a screenreader is available.
        /// </summary>
        public static bool IsAvailable => _available;

        #endregion
    }
}
